...
void blocks_init(const char *image_path) {
  ...
  // block 1 stores the inode bitmap  // ADDED & CHECKED
  bitmap_put(bbm, 1, 1);

  // block 2-n stores the inode table   // ADDED & CHECKED
...
void *get_inode_bitmap() {return blocks_get_block(0) + BLOCK_BITMAP_SIZE;}
